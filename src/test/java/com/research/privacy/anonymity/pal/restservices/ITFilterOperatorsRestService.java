...
class ITFilterOperatorsRestService {
    ...
    void privacyService_getFilterOperations_OK() {
        final ResponseEntity<List<String>> restServiceResponse = filterOperationsRestService.getFilterOperations();
        Assertions.assertEquals(HttpStatus.OK, restServiceResponse.getStatusCode());
        final List<String> filterOperations = restServiceResponse.getBody();
